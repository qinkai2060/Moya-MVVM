...
@interface WARPageCoverController ()<UIGestureRecognizerDelegate>
...
@implementation WARPageCoverController
...
- (void)touchPan:(UIPanGestureRecognizer *)pan
{
    ...
    CGFloat absX = fabs(tempPoint.x);
    CGFloat absY = fabs(tempPoint.y);
        ...
        if (self.isPan) {
            ...
            if (self.openAnimate) { // 动画
                ...
                if (self.pendingController) {
                    ...
                    // (第一页继续向前滑动，或最后一张向后滑动)代理回调
                    if ([self.delegate respondsToSelector:@selector(coverController:currentController:finish:)]) {
                        
                        [self.delegate coverController:self currentController:self.currentController finish:NO];
                    }
                }
    ...
    if (self.pendingController) {
        ...
        if (self.isLeft) { // 左边
            ...
            if (animated) {
                ...
                __weak WARPageCoverController *weakSelf = self;
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldRecognizeSimultaneouslyWithGestureRecognizer:(UIGestureRecognizer *)otherGestureRecognizer
{
    ...
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]] && [gestureRecognizer isEqual:self.pan]) {
        
        CGPoint tempPoint = [self.pan translationInView:self.view];
...
- (void)dealloc {
    ...
    if (self.currentController) {
        if (self.currentController.view) {
            [self.currentController.view removeFromSuperview];
            [self.currentController removeFromParentViewController];
            _currentController = nil;
        }
