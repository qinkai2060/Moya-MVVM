...
@interface WARMapProfileMomentListVC ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,WARFriendBaseCellDelegate,WARPhotoBrowserDelegate>
...
@implementation WARMapProfileMomentListVC
...
- (void)loadDataRefresh:(BOOL)refresh {
    ...
    [WARJournalFriendCycleNetManager loadMapProfileMomentListWithAccountId:self.accountId
                                                             categoryLabel:self.categoryLabel
                                                                       lat:self.lat
                                                                       lon:self.lon
                                                                  momentId:self.momentId
                                                                      zoom:self.zoom
                                                                lastFindId:self.lastFindId
                                                               compeletion:^(WARMomentModel *model, NSArray<WARMoment *> *results, NSError *err) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (!err && results.count > 0) {
            strongSelf.lastFindId = model.searchSort;
        }
        if (refresh) {
