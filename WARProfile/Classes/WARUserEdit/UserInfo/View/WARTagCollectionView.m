...
- (NSInteger)indexOfTagAt:(CGPoint)point {
    // We expect the point to be a point wrt to the WARTagCollectionView.
...
- (void)layoutTagViews {
    ...
    if (_scrollDirection == WARTagCollectionScrollDirectionVertical) {
    ...
    for (NSUInteger currentLine = 0; currentLine < numberOfLines; currentLine++) {
        ...
        switch (_alignment) {
            ...
            case WARTagCollectionAlignmentFillByExpandingSpace:
                ...
                break;
            case WARTagCollectionAlignmentFillByExpandingWidth:
                currentLineXOffset = _contentInset.left;
        ...
        [eachLineTagIndexs[currentLine] enumerateObjectsUsingBlock:^(NSNumber * _Nonnull tagIndexNumber, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (_scrollDirection == WARTagCollectionScrollDirectionVertical && tagSize.width > maxLineWidth) {
