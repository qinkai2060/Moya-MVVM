...
/** 播放器的播放状态 */
typedef NS_ENUM(NSInteger, TracyVideoPlayerState) {
...
@interface TracyVideoPlayer()<TracyPlaybackControlsDelegate>
...
/** 播放器配置信息 */
@property (nonatomic, strong) TracyPlayerConfiguration *playerConfiguration;
/** 非全屏状态下播放器 superview */
...
/** 播放器的播放状态 */
@property (nonatomic, assign) TracyVideoPlayerState playerState;
/** 是否结束播放 */
@property (nonatomic, assign) BOOL playDidEnd;

@end

@implementation TracyVideoPlayer
...
- (void)createTimer {
    ...
    self.timeObserve = [self.player addPeriodicTimeObserverForInterval:CMTimeMakeWithSeconds(1, 1) queue:nil usingBlock:^(CMTime time){
        ...
        if (loadedRanges.count > 0 && currentItem.duration.timescale != 0) {
            ...
            CGFloat value = CMTimeGetSeconds([currentItem currentTime]) / (NSInteger)totalTime;
 ...
 */
- (void)_setVideoGravity:(TracyVideoGravity)videoGravity
{
 ...
 */
- (void)setPlayerState:(TracyVideoPlayerState)playerState
{
...
#pragma mark 滑杆拖动代理
/** 开始拖动 */
-(void)videoSliderTouchBegan:(TracyVideoSlider *)slider{
    [self _pauseVideo];
    [_playbackControls _playerCancelAutoHidePlaybackControls];
}
/** 结束拖动 */
-(void)videoSliderTouchEnded:(TracyVideoSlider *)slider{
...
/** 拖拽中 */
-(void)videoSliderValueChanged:(TracyVideoSlider *)slider{
