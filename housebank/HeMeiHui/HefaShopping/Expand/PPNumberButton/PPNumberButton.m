...
{
    if (self.dataSource.count>0&&_currentResult==nil) {
        [self.delegate showReluerView:@"1" storck:nil];
        textField.text=@"1";
        return;
    }
    if (self.maxValue>0&&[textField.text integerValue]>self.maxValue) {
        [self.delegate showReluerView:@"2" storck:self.maxValue];
        textField.text=[NSString stringWithFormat:@"%ld",(long)self.maxValue];
      
    }else
    {
        if (_currentResult&& [textField.text integerValue]> [_currentResult[@"stock"] integerValue]) {
            [self.delegate showReluerView:@"3"storck:[_currentResult[@"stock"] integerValue]];
            textField.text=[NSString stringWithFormat:@"%@",_currentResult[@"stock"]];
         
        }
        if (!_currentResult&&[textField.text integerValue]>self.storkNum) {
            [self.delegate showReluerView:@"3" storck:self.storkNum];
            textField.text=[NSString stringWithFormat:@"%ld",self.storkNum];
...
//    if (self.maxValue>0&&[str integerValue]>self.maxValue) {
//        [self.delegate showReluerView:@"2"];
//        return NO;
//    }else
//    {
//        if (_currentResult&& [str integerValue]> [_currentResult[@"stock"] integerValue]) {
//            [self.delegate showReluerView:@"3"];
//            return NO;
//        }
//        if (!_currentResult&&[str integerValue]>self.storkNum) {
//            [self.delegate showReluerView:@"3"];
//            return NO;
//        }
//    }
...
- (void)increase
{
    ...
    if (number <= _maxValue)
    {
        ...
        if (self.maxValue>0&&number>self.maxValue) {
            [self.delegate showReluerView:@"2" storck:self.maxValue];
             return;
        }else
        {
            if (_currentResult&&number> [_currentResult[@"stock"] integerValue]) {
                [self.delegate showReluerView:@"3"storck:[_currentResult[@"stock"] integerValue]];
                 return;
            }else
            {
                if (!_currentResult&&number>self.storkNum) {
                    [self.delegate showReluerView:@"3"storck:self.storkNum];
                     return;
                }else
                {
...
- (void)buttonClickCallBackWithIncreaseStatus:(BOOL)increaseStatus
{
    if ([self.textField.text integerValue]<=1) {
        self.decreaseImage=[UIImage imageNamed:@"减号禁用"];
    }else
    {
        self.decreaseImage=[UIImage imageNamed:@"减号可用"];
    }
