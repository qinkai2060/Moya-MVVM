...
@interface HFFindPassWordView()<HFTextFieldDelegate,NTESVerifyCodeManagerDelegate>
...
@property(nonatomic,strong)UIView *lineLayer5;
...
@end
@implementation HFFindPassWordView{
...
- (void)hh_setupViews {
    ...
    self.passWordTextField.frame = CGRectMake(30, self.lineLayer3.bottom, ScreenW-60, 60);
    self.lineLayer4.frame = CGRectMake(30, self.passWordTextField.bottom, ScreenW-60, 0.5);
    [self addSubview:self.surepassWordTextField];
    [self addSubview:self.lineLayer5];
    self.surepassWordTextField.frame = CGRectMake(30, self.lineLayer4.bottom, ScreenW-60, 60);
    self.lineLayer5.frame = CGRectMake(30, self.surepassWordTextField.bottom, ScreenW-60, 0.5);
    [self addSubview:self.agreePolicyBtn];
    [self addSubview:self.agreePolicyLb];
    self.agreePolicyBtn.frame = CGRectMake(30, self.lineLayer5.bottom+20, 20, 20);
    self.agreePolicyLb.frame = CGRectMake(self.agreePolicyBtn.right+5, self.lineLayer5.bottom+22, ScreenW-30-self.agreePolicyBtn.right-10, 15);
    [self.layer addSublayer:self.gradientlayer];
    [self addSubview:self.loginBtn];
    self.loginBtn.frame = CGRectMake(30, self.lineLayer5.bottom+10, ScreenW-60, 50);
...
- (void)hh_bindViewModel {
    @weakify(self)
    RAC(self.viewModel, findPhone)= self.userNametextField.rac_textSignal;
    RAC(self.viewModel,findCode) = self.codeTextField.rac_textSignal;
    RAC(self.viewModel,findPassWord) = self.passWordTextField.rac_textSignal;
    RAC(self.viewModel,findSurePassWord) = self.surepassWordTextField.rac_textSignal;
    [[self.userNametextField rac_textSignal] subscribeNext:^(NSString * _Nullable x) {
        if ((![self.phone isEqualToString:self.viewModel.findPhone]) || self.viewModel.userName.length == 0) {
            [[HFGCDManger sharedInstance] cancelTimerWithName:@"HFFindPassWordView"];
    ...
    [self.surepassWordTextField.rac_textSignal subscribeNext:^(NSString * _Nullable x) {
        NSString *toBeString = x;
        
        if (toBeString.length >= 20) {
            self.surepassWordTextField.text = [toBeString substringToIndex:20];
            [MBProgressHUD showAutoMessage:@"超过限制字符数"];
        }
    ...
    [[self.coverBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
     ...
     RAC(self.loginBtn,enabled)   = self.viewModel.findValidSigal;
    RAC(self.loginBtn,backgroundColor) = [self.viewModel.findValidSigal map:^id _Nullable(id  _Nullable value) {
        if ([value boolValue]) {
    ...
    [[self.loginBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        @strongify(self)
        [self endEditing:YES];
//        if (self.viewModel.findAgree) {
            if ([HFUntilTool isValidateByRegex:self.viewModel.findPhone]) {
                if (self.viewModel.findSurePassWord.length <8 ||self.viewModel.findPassWord.length <8) {
                    [MBProgressHUD showAutoMessage:@"请输入8-20位数字和字母密码组合"];
                }else {
                    if (![self.viewModel.findSurePassWord isEqualToString:self.viewModel.findPassWord]) {
                        [MBProgressHUD showAutoMessage:@"两次输入的密码不一致"];
                    }else {
                        [self.viewModel.findCommnd execute:nil];
                    }
...
//        }
    ...
    [[self.agreePolicyBtn rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
        ...
        self.viewModel.findAgree = self.agreePolicyBtn.selected;
    }];
    ...
    [[self rac_signalForSelector:@selector(verifyCodeValidateFinish:validate:message:) fromProtocol:@protocol(NTESVerifyCodeManagerDelegate)] subscribeNext:^(RACTuple * _Nullable x) {
        ...
        if([[x first] boolValue]) {
            if ([x allObjects].count >= 3) {
                self.viewModel.findNECaptchaValidate = [[x allObjects] objectAtIndex:1];
            }
            if (self.viewModel.findNECaptchaValidate) {
                [self codeValite];
                [self.viewModel.findsendCodeCommnd execute:nil];
    ...
    [self.viewModel.didSelectCountryCodeSubject subscribeNext:^(id  _Nullable x) {
        ...
        self.viewModel.findcountryCode = model.countryCode;
...
- (void)codeValite {
    ...
    @weakify(self)
    [[HFGCDManger sharedInstance] scheduledDispatchTimerWithName:@"HFFindPassWordView" timeInterval:1 queue:nil repeats:YES fireInstantly:YES action:^{
        @strongify(self)
        count--;
...
- (UIView *)lineLayer5 {
    if (!_lineLayer5) {
        _lineLayer5 = [[UIView alloc] init];
        _lineLayer5.backgroundColor = [UIColor colorWithHexString:@"D5D5D5"];
    }
    return _lineLayer5;
}
...
- (UITextField *)surepassWordTextField {
    if (!_surepassWordTextField) {
        ...
        _surepassWordTextField.font = [UIFont systemFontOfSize:17];
...
- (UIButton *)phoneRegBtn {
    if (!_phoneRegBtn) {
        ...
        [_phoneRegBtn setTitle:@"找回密码" forState:UIControlStateNormal];
