...
- (void)didSelectedPropertyWithIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!lastIndexPath) {
        ...
        [_availableIndexPathsSet intersectSet:[self availableIndexPathsFromSelctedIndexPath:indexPath sectedIndexPaths:self.selectedIndexPaths]];
        [self updateCurrentResult];
        return;
    }
    ...
    if (lastIndexPath.item != indexPath.item) {
        //切换属性
        [self.selectedIndexPaths addObject:indexPath];
        [self.selectedIndexPaths removeObject:lastIndexPath];
        [self updateAvailableIndexPaths];
        [self updateCurrentResult];
    }
...
- (void)updateCurrentResult {
    ...
    if (self.selectedIndexPaths.count != [_dataSource numberOfSectionsForPropertiesInFilter:self]) {
        _currentResult = nil;
        return;
    }
