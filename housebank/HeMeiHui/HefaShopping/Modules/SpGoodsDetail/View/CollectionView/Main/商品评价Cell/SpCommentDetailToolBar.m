...
@interface SpCommentDetailToolBar ()<UITextViewDelegate, DXFaceDelegate>
{
...
@property (nonatomic, strong) UIView *bgInputTextView;
...
@implementation SpCommentDetailToolBar
...
//        }
    ...
    if (remarkStr.length > 0) {
        _faceButton.enabled = YES;
        [_faceButton setBackgroundColor:RGBACOLOR(251, 2, 66, 1)];
    } else {
        _faceButton.enabled = NO;
        [_faceButton setBackgroundColor:RGBACOLOR(213, 213, 213, 1)];
    }

//    if (remarkStr.length >= 1) {
//        self.faceButton.enabled = YES;
//        self.faceButton.backgroundColor = RGBACOLOR(251, 2, 66, 1);
//    } else {
//        self.faceButton.enabled = NO;
//        self.faceButton.backgroundColor = RGBACOLOR(213, 213, 213, 1);
//    }
...
- (void)setupConfigure{
    ...
    [self.toolbarView setBackgroundColor:[UIColor colorWithRed:235.0/255 green:235.0/255 blue:235.0/255 alpha:1.0f]];
...
- (void)setupSubviews{
    ...
    // 发送
    self.faceButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kHorizontalPadding - 70, kVerticalPadding, 70, kInputTextViewMinHeight)];
    ...
    self.faceButton.titleLabel.font = [UIFont systemFontOfSize:14.0];
    ...
    self.faceButton.layer.cornerRadius = self.faceButton.frame.size.height / 2;
    
    // 输入框的高度和宽度
    //    CGFloat width = CGRectGetWidth(self.bounds) - (allButtonWidth ? allButtonWidth : (textViewLeftMargin * 2));
    inputWidth = CGRectGetWidth(self.bounds) - 20 - 70;
    
    [self.toolbarView addSubview:self.faceButton];
    
    self.bgInputTextView = [[UIView alloc] initWithFrame:CGRectMake(textViewLeftMargin, kVerticalPadding, inputWidth, kInputTextViewMinHeight)];
    self.bgInputTextView.backgroundColor = [UIColor whiteColor];
    self.bgInputTextView.layer.masksToBounds = YES;
    self.bgInputTextView.layer.cornerRadius = kInputTextViewMinHeight/2;
    [self.toolbarView addSubview:self.bgInputTextView];

    
    // 初始化输入框
    self.inputTextView = [[XHMessageTextView  alloc] initWithFrame:CGRectMake(textViewLeftMargin + 3, kVerticalPadding , inputWidth - 6, kInputTextViewMinHeight)];
...
//    }
    ...
    _inputTextView.placeHolder =@"请输入评价";
    _inputTextView.font = [UIFont systemFontOfSize:14.0];
    _inputTextView.delegate = self;
    _inputTextView.backgroundColor = [UIColor clearColor];
//    _inputTextView.layer.masksToBounds = YES;
//    _inputTextView.layer.cornerRadius = kInputTextViewMinHeight/2;
...
//        }
    ...
    toFrame = CGRectMake(fromFrame.origin.x, fromFrame.origin.y + (fromFrame.size.height - toHeight) + hF, fromFrame.size.width, toHeight);
...
- (void)willShowInputTextViewToHeight:(CGFloat)toHeight
{
    ...
    if (toHeight == _previousTextViewContentHeight){
        ...
        self.bgInputTextView.height = self.toolbarView.height -kVerticalPadding * 2;
//        self.inputTextView.height = self.bgInputTextView.height;
