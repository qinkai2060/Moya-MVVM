...
@interface SpTypesSearchListMainView ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation SpTypesSearchListMainView
...
- (void)createViewWithSearchStr:(NSString *)searchStr{
    //
    self.searchView = [[SpTypesSearchView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 44) isAddOneBtn:YES addBtnImageName:@"spTypes_search_collection" addBtnTitle:@"" searchKeyStr:searchStr canEidt:NO placeholderStr:@"" isHaveBack:YES isHaveBottomLine:NO];
    ...
    //这种是自定义cell不带xib的注册
    [_collectionView registerClass:[SpTypesSearchListOneCollectionViewCell class] forCellWithReuseIdentifier:@"OneCollectionCell"];
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    if (self.isListView) {
        _collectionView.backgroundColor= [UIColor whiteColor];
        SpTypesSearchListOneCollectionViewCell *cell=[collectionView dequeueReusableCellWithReuseIdentifier:@"OneCollectionCell" forIndexPath:indexPath];
        
        if (self.dataSource.count > indexPath.row) {
            GetProductListByConditionModel *model = self.dataSource[indexPath.row];
            model.cellIndexRow = indexPath.row;
            [cell refreshCellWithModel:model];
            
            // 进店
            __weak typeof( self)weakSelf=self;
            cell.toShopBtnClickBlock = ^(SpTypesSearchListOneCollectionViewCell * _Nonnull cell, NSInteger cellIndexRow) {
                if (weakSelf.dataSource.count > cellIndexRow) {
                    GetProductListByConditionModel *model = weakSelf.dataSource[cellIndexRow];
