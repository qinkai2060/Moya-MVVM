...
@interface AssembleSearchView ()<UITextFieldDelegate>
...
@implementation AssembleSearchView
...
-(void)createNav{
    ...
    if (self.isAddBtn) {
        if (self.isHaveBack) { // 判断是否有返回按钮
            bgView.frame = CGRectMake(40, 7, ScreenW - 60 - 30+ 30, 30);
        } else {
    ...
    _searchTextField = [[UITextField alloc] initWithFrame:CGRectMake(35,0, bgView.frame.size.width - 35, bgView.frame.size.height)];
    ...
    if (self.searchKeyStr.length > 0) {
        _searchTextField.text = self.searchKeyStr;
    }else {
        if (self.placeholderStr.length > 0) {
            _searchTextField.placeholder = self.placeholderStr;
        } else {
