...
- (void)setupScanningQRCode {
    ...
    if(authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied){
        SGAlertView *alertView = [SGAlertView alertViewWithTitle:@"⚠️ 警告" delegate:nil contentTitle:@"请先打开相机访问权限" alertViewBottomViewType:(SGAlertViewBottomViewTypeOne)];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        // 扫描结果为条形码
                if (self.delegate&&obj.stringValue.length>0) {
                    NSString *body = self.body;
                    __block /*主控制器*/SGScanningQRCodeVC *weakSelf = self;
             
                    dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5/*延迟执行时间*/ * NSEC_PER_SEC));
                    dispatch_after(delayTime, dispatch_get_main_queue(), ^{
                        [weakSelf.delegate afterScan:obj.stringValue funcName:body];
