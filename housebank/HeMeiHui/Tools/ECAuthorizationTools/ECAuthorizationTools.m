...
//                [ECAuthorizationTools executeCallBack:accessStatusCallBack accessStatus:ECAuthorizationStatus_NotDetermined type:ECPrivacyType_Health];
                ...
                [self.healthStore requestAuthorizationToShareTypes:typeSet readTypes:typeSet completion:^(BOOL success, NSError * _Nullable error) {
                    ...
                    if (success) {
                        ...
                        HKAuthorizationStatus status = [self.healthStore authorizationStatusForType:heartRateType];
                        if (status == HKAuthorizationStatusNotDetermined) {
                            [ECAuthorizationTools executeCallBack:accessStatusCallBack accessStatus:ECAuthorizationStatus_NotDetermined type:ECPrivacyType_MotionAndFitness];

                        } else if (status == HKAuthorizationStatusSharingAuthorized) {
                            [ECAuthorizationTools executeCallBack:accessStatusCallBack accessStatus:ECAuthorizationStatus_Authorized type:ECPrivacyType_MotionAndFitness];

                        } else {
                            // HKAuthorizationStatusSharingDenied
                            [ECAuthorizationTools executeCallBack:accessStatusCallBack accessStatus:ECAuthorizationStatus_Denied type:ECPrivacyType_MotionAndFitness];
                        }
                    }else{
                        if (error) {
            ...
            [ECAuthorizationTools executeCallBack:accessStatusCallBack accessStatus:ECAuthorizationStatus_NotSupport type:ECPrivacyType_MotionAndFitness];
            NSLog(@"unavailable");
