...
//    self.titleLabel.textAlignment = NSTextAlignmentCenter;
    ...
    self.lockBtn.frame = CGRectMake(10, self.contentView.frame.size.height / 2 - 25, 50, 50);
...
//    if([[UIDevice currentDevice] systemVersion].intValue>=10){ // 增加下面这行可以解决iOS10兼容性问题了
...
//        int32_t timeScale = self.player.currentItem.asset.duration.timescale;
        ...
        [self.player seekToTime:CMTimeMakeWithSeconds(time, _currentItem.currentTime.timescale) toleranceBefore:kCMTimeZero toleranceAfter:kCMTimeZero completionHandler:^(BOOL finished) {
            if (finished) { // 此处是为了解决 当快进结束之后 回到0秒时 暂停
                if (time == 0.0) {
                    [self showControlView];
                    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                        self.state = WMPlayerStateFinished;
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    // 如果是声音和亮度 快近 就不做其他操作
//    if (_isLightJudge || _controlJudge || _isVolmControl) {
//        return;
//    }
...
//                }
    ...
    if (_controlType == progressControl && _controlJudge) {     //如果是进度手势
        float value = [self moveProgressControllWithTempPoint:tempPoint];
        [self timeValueChangingWithValue:value];
        
        return;
        
    } else if(_controlType == voiceControl && _isVolmControl){    //如果是音量手势
    ...
    }else if(_controlType == lightControl && _isLightJudge){   //如果是亮度手势
        //显示音量控制的view
...
NSString * calculateTimeWithTimeFormatter1(long long timeSecond){
