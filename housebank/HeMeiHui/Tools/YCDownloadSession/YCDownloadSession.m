...
// NetStateDelegete    UIAlertViewDelegate
/**正在下载的task*/
...
@property (nonatomic, strong) NSString *notifiFiledId;
...
- (void)NetWorkReachablityWithFileId:(NSString *)fileld
{
    self.notifiFiledId = fileld;
    AFNetworkReachabilityManager *manager = [AFNetworkReachabilityManager sharedManager];
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusReachableViaWiFi://2 wifi
            {
                //NSLog(@"WiFi");
            }
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN://1  手机网络
            {
                // 解决了为手机网络时 不下载的问题
                if (self.isAllowsCellularAccess) {
                    [self allowsCellularAccess:YES];
                }
                if (!self.isAllowsCellularAccess) { // 如果不允许使用手机网络 则发通知
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"当前网络为手机网络，是否继续下载？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
                    alert.tag = 9999;
                    [alert show];
                }
            }
            ...
            case AFNetworkReachabilityStatusUnknown://-1
                // NSLog(@"未知网络");
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"networkChangedWWANorNotRechable" object:[NSString stringWithFormat:@"%@",fileld]];
//
                break;
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (alertView.tag == 9999) { //网络为4G
        if (buttonIndex == 1) { // 确定
            ...
            NSDictionary *dic = @{@"filedId":self.notifiFiledId,@"stated":@"1"};
            [[NSNotificationCenter defaultCenter] postNotificationName:@"networkChangedWWANorNotRechable" object:dic];
        }
    } else { // 无网络
        NSDictionary *dic = @{@"filedId":self.notifiFiledId,@"stated":@"2"};
        [[NSNotificationCenter defaultCenter] postNotificationName:@"networkChangedWWANorNotRechable" object:dic];
    }
}
...
//    _netTool = [[NetStatTool alloc] init];
//    [_netTool NetWorkReachablity];
//    _netTool.delegete = self;
    
    // 先判断网络
    [self NetWorkReachablityWithFileId:fileId];
...
//    if (state == 2) { // wifi
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"wifi =====" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//        [alert show];
//    } else if (state == 1){ // 手机网络
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"当前网络为手机网络，是否继续下载？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
//        [alert show];
//
//    } else if (state == 0){ // 无网络
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"无网络  =====" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//        [alert show];
//
//    } else if (state == -1){ // 未知网络
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"未知网络 =====" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//        [alert show];
//    }
//}
