...
@interface HFDescoveAsyncLayout()
...
@end
@implementation HFDescoveAsyncLayout
- (instancetype)init {
    if (self = [super init]) {
        self.columnCount = 2;
    }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat itemWidth = (collectionViewWidth - 10 - 10 - 5) / 2;
    ...
    //根据最短列的列数计算item的x值
    CGFloat itemX = 10 + (5 + itemWidth) * minIndex.integerValue;
    
    //item的y值 = 最短列的最大y值 + 行间距
    CGFloat itemY = [self.maxYDic[minIndex] floatValue] + 5;
...
- (CGSize)collectionViewContentSize {
    ...
    return CGSizeMake(0, [self.maxYDic[maxIndex] floatValue] + 20);
}
