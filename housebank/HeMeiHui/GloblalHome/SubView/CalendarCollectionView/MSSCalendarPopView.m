...
//static const CGFloat CalendarPopViewArrowHeight = 0.0f;
...
- (void)createUI
{
    ...
    _backgroundView.backgroundColor = [HEXCOLOR(0x000000) colorWithAlphaComponent:1];
//    _backgroundView.layer.cornerRadius = 5.0f;
    [self addSubview:_backgroundView];
    
    _topLabel = [[UILabel alloc]init];
    _topLabel.textColor = HEXCOLOR(0xFFFFFF);
    _topLabel.font = [UIFont boldSystemFontOfSize:12.0f];
    _topLabel.textAlignment = NSTextAlignmentCenter;
    [_backgroundView addSubview:_topLabel];
    
    _bottomLabel = [[UILabel alloc]init];
    _bottomLabel.textColor = HEXCOLOR(0xFFFFFF);
...
- (void)drawRect:(CGRect)rect
{
    ...
    [path addLineToPoint:CGPointMake(_drawArrowStartX + CalendarPopViewArrowHeight, rect.size.height - CalendarPopViewArrowHeight)];
    [path closePath];
    [[HEXCOLOR(0x000000) colorWithAlphaComponent:1]setFill];
    [path fill];
}
...
- (void)updateUI
{
    ...
    CGFloat width = MAX(topTextRect.size.width, bottomTextRect.size.width) + 10;
    ...
    self.frame = CGRectMake(x, _sideViewInWindowRect.origin.y - height - CalendarPopViewArrowHeight-CalendarPopViewArrowHeight-CalendarPopViewArrowHeight, width, height + CalendarPopViewArrowHeight);
    _backgroundView.frame = CGRectMake(0, 0, width, height);
    _topLabel.frame = CGRectMake(0, 4, width, 15);
