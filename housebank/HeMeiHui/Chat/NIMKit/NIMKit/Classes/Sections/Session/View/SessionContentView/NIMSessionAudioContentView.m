...
- (void)addVoiceView{
    //    voiceImage = [UIImage nim_imageInKit:@"icon_receiver_voice_playing.png"];
    //    _voiceImageView = [[UIImageView alloc] initWithImage:voiceImage];
...
- (void)refresh:(NIMMessageModel *)data{
    ...
    for (UIView *vi in self.subviews) {
        if ([vi isEqual:_voiceImageView] || [vi isEqual:_durationLabel]) {
            [vi removeFromSuperview];
        }
    }
#pragma mark 此处用来判断发送语音时播放语音的动画  isOutgoingMsg 是来表示是自己发的还是对方发的消息  以此来布局左右
    if (!data.message.isOutgoingMsg) {
        ...
        voiceImage = [UIImage nim_imageInKit:@"icon_receiver1_voice_playing.png"];
        _voiceImageView = [[UIImageView alloc] initWithImage:voiceImage];
        animateNames = @[@"icon_receiver1_voice_playing_001.png",@"icon_receiver1_voice_playing_002.png",@"icon_receiver1_voice_playing_003.png"];
    }
