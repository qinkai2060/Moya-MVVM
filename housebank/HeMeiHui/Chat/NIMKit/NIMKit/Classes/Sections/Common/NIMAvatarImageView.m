...
//    [self nim_setImageWithURL:url placeholderImage:info.avatarImage];
        [self nim_setImageWithURL:url placeholderImage:[UIImage imageNamed:@"DefaultAvatar"]];
}
...
- (void)nim_setImageWithURL:(NSURL *)url placeholderImage:(UIImage *)placeholder options:(SDWebImageOptions)options progress:(SDWebImageDownloaderProgressBlock)progressBlock completed:(SDExternalCompletionBlock)completedBlock {
    ...
    if (!(options & SDWebImageDelayPlaceholder)) {
        dispatch_main_async_safe(^{
            [self nim_setImage:placeholder imageData:nil withUrl:url basedOnClassOrViaCustomSetImageBlock:nil];
        });
    }
...
//            [sself sd_removeActivityIndicator];
            ...
            dispatch_main_async_safe(^{
                ...
                if (image && (options & SDWebImageAvoidAutoSetImage) && completedBlock) {
                    ...
                    [sself nim_setImage:image imageData:data withUrl:url basedOnClassOrViaCustomSetImageBlock:nil];
                    [sself nim_setNeedsLayout];
                } else {
                    if ((options & SDWebImageDelayPlaceholder)) {
                        [sself nim_setImage:placeholder imageData:nil withUrl:url basedOnClassOrViaCustomSetImageBlock:nil];
                        [sself nim_setNeedsLayout];
                    }
                }
...
- (void)nim_setImage:(UIImage *)image imageData:(NSData *)imageData withUrl:(NSURL *)imageUrl basedOnClassOrViaCustomSetImageBlock:(SDSetImageBlock)setImageBlock {
    if (setImageBlock) {
        setImageBlock(image, imageData,SDImageCacheTypeAll,imageUrl);
        return;
    }
    self.image = image;
}
