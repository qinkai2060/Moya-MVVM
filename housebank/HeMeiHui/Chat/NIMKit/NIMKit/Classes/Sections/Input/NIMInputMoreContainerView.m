...
#pragma mark 此处修改点击加号时显示的格式 **************
    NSInteger count = 2 / NIMMaxItemCountInPage;
    count = (2 % NIMMaxItemCountInPage == 0) ? count: count + 1;
#pragma mark 此处修改点击加号时显示的格式 **************
    return MAX(count, 1);
}
...
#pragma mark 此处设置点击加好时  键盘上现实的样式  ========================
    
    //    return [self mediaPageView:pageView beginItem:begin endItem:end];
    return [self mediaPageView:pageView beginItem:begin endItem:2];
